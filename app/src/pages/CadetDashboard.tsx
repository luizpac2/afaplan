...
export const CadetDashboard = () => {
  ...
  // Número do esquadrão: cohort com entryYear = anoFiltro é 1º, anoFiltro-1 é 2º, etc.
  const squadronNumber = (entryYear: number) => anoFiltro - entryYear + 1;

  // 4 esquadrões ordenados por posição no anoFiltro (1º ao 4º)
  const bySquadron = useMemo(() => {
    const acc: Record<string, {
      total: number; CFOAV: number; CFOINT: number; CFOINF: number;
      name: string; num: number; cohortId: string;
    }> = {};

    ativos.forEach((c) => {
      if (!acc[c.cohort_id]) {
        const co = cohortMap.get(c.cohort_id);
        const num = co ? squadronNumber(co.entryYear) : 99;
        acc[c.cohort_id] = {
          total: 0, CFOAV: 0, CFOINT: 0, CFOINF: 0,
          name: co?.name ?? `Esq. ${c.cohort_id}`,
          num,
    ...
    // Garante os 4 esquadrões esperados, mesmo sem ativos
    cohorts.forEach((co) => {
      if (!acc[co.id]) {
        const num = squadronNumber(co.entryYear);
        if (num >= 1 && num <= 4) {
          acc[co.id] = { total: 0, CFOAV: 0, CFOINT: 0, CFOINF: 0, name: co.name, num, cohortId: co.id };
        }
      }
    });

    return Object.values(acc)
      .filter((s) => s.num >= 1 && s.num <= 4)
      .sort((a, b) => a.num - b.num);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ativos, cohortMap, cohorts, anoFiltro]);

  // Dados para o gráfico com label "Nº Esq — Nome"
  const chartData = bySquadron.map((s) => ({
    ...s,
    label: `${s.num}º — ${s.name}`,
  }));
  ...
  const StackedTotalLabel = (props: Record<string, unknown>) => {
    ...
    return (
      <text x={x + width / 2} y={y - 5} textAnchor="middle" fontSize={11} fontWeight={600} fill={labelFill}>
        {total}
      </text>
    );
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-sm ${lbl}`}>Turma em:</span>
          <select
            value={anoFiltro}
      ...
      {loading ? (
        ...
        <>
          {/* KPI Cards — 1 total + 4 esquadrões */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            ...
            {bySquadron.map((s) => (
              <div key={s.cohortId} className={card}>
                <div className="flex items-center gap-3 mb-2">
                  <div className="p-2 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                    <GraduationCap size={18} className="text-indigo-600 dark:text-indigo-400" />
                  </div>
                  <span className={`text-xs uppercase tracking-wide ${lbl}`}>
                    {s.num}º Esq — {s.name}
                  </span>
                </div>
                <p className={`text-3xl font-light ${dark ? 'text-slate-100' : 'text-slate-900'}`}>{s.total}</p>
                <p className={`text-xs mt-1 ${lbl}`}>
                  {ativos.length ? Math.round((s.total / ativos.length) * 100) : 0}% do total
                </p>
              </div>
            ))}
          </div>
            ...
            {byQuadro.map(({ quadro, label: ql, value }) => (
              <div key={quadro} className="flex items-center gap-3">
                <div className="p-2 rounded-lg" style={{ backgroundColor: QUADRO_COLORS[quadro] + '22' }}>
                  <GraduationCap size={16} style={{ color: QUADRO_COLORS[quadro] }} />
                </div>
                <div>
                  <p className={`text-xs uppercase tracking-wide ${lbl}`}>{ql}</p>
                  <p className={`text-xl font-light ${dark ? 'text-slate-100' : 'text-slate-900'}`}>{value}</p>
                </div>
              </div>
            ))}
          </div>
